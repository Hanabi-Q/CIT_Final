...
class TrainingConfig:
    ...
    output_dir: str = "resnet_training_results"
...
class ResNetSR(nn.Module):
    ...
    def __init__(self, config: TrainingConfig):
        ...
        self.input_conv = nn.Sequential(
            nn.Conv2d(in_channels, base_channels, kernel_size=9, padding=4, bias=False),
        ...
        self.output_conv = nn.Conv2d(base_channels, out_channels, kernel_size=9, padding=4)

        self._initialize_weights()

        logger.info(f"ResNet-SR initialized with {self._count_parameters():,} parameters")
        logger.info(f"Architecture: {num_blocks} residual blocks, {base_channels} base channels")
    ...
    def _initialize_weights(self):
        ...
        nn.init.kaiming_normal_(self.output_conv.weight, mode='fan_in', nonlinearity='linear', a=0.1)
...
class ResNetTrainer:
    ...
    def _setup_model(self):
        self.model = ResNetSR(self.config).to(self.device)
...
def main():
    ...
    config = TrainingConfig(
        ...
        output_dir="resnet_training_results",
