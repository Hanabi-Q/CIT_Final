...
class ResolutionSimulator:
    def __init__(self, pixel_spacing: float = 1.0,
        ...
        logger.info("Using CUBIC interpolation for better structure preservation")
...
class SR4ZCTDatasetGenerator:
    ...
    def __init__(self, output_directory: str = "sr4zct_cubic_dataset",
    ...
    def generate_training_data(self, input_volume_path: str,
        ...
        logger.info("=== Starting SR4ZCT Dataset Generation (CUBIC INTERPOLATION) ===")
        ...
        logger.info("Generating vertically degraded images (CUBIC)...")
        ...
        logger.info("Generating horizontally degraded images (CUBIC)...")
    ...
    def _process_volume_slices(self, volume: np.ndarray, direction: str) -> np.ndarray:
        ...
        desc = f"{direction.capitalize()} degradation (cubic)"
    ...
    def _generate_test_volume(self, reference_volume: np.ndarray) -> np.ndarray:
        ...
        for col_idx in tqdm(range(reference_volume.shape[1]), desc="Generating test data (cubic)"):
    ...
    def _generate_dataset_summary(self, final_dimensions: Tuple[int, int, int]) -> dict:
        ...
        with open(summary_path, 'w', encoding='utf-8') as f:
            f.write("SR4ZCT Dataset Summary (CUBIC INTERPOLATION)\n")
            f.write("=" * 50 + "\n\n")

            f.write("Interpolation Method: CUBIC\n")
            f.write("This dataset uses cubic interpolation instead of linear interpolation\n")
            f.write("for better structure preservation and edge detail retention.\n\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Generate SR4ZCT training dataset with cubic interpolation')
    parser.add_argument('input_volume', help='Path to input reconstructed volume (.npy)')
    parser.add_argument('-o', '--output', default='sr4zct_cubic_dataset',
    ...
    try:
        ...
        logger.info("\n" + "=" * 20)
        logger.info("CUBIC dataset generation completed successfully!")
        ...
        logger.error(f"CUBIC dataset generation failed: {e}")
        raise
